...
void insert(Listptr *p,int pos, int val) {
    ...
    newnode -> next = curr -> next; //πρώτα φτιάχνω το δεξί βέλος
    curr->next = newnode;
...
void delete_el(Listptr *p, int val) {
    if(*p &&(*p)->data == val) {
        Listptr temp = *p; //Το *p δείχνει στον κόμβο που θέλω να διαγράψω τώρα
        *p = temp->next;
        free(temp);
        return;
    }
...
Listptr reverse(Listptr p) {
    ...
    while(curr != NULL) {
        ...
        curr->next = prev; //το επόμενο θα γίνει το προηγούμενο
        prev = curr; //το προηγούμενο θα γίνει το τρέχον
        curr = next; // και το τρέχον θα γίνει το επόμενο
    }
